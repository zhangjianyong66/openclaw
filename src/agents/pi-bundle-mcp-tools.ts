        ...
        for (const tool of listedTools) {
          ...
          tools.push({
            ...
            execute: async (_toolCallId, input) => {
              // 从服务器配置中读取超时设置（秒转换为毫秒）
              const serverToolTimeout = rawServer.toolTimeout;
              const timeout =
                typeof serverToolTimeout === "number" && Number.isFinite(serverToolTimeout)
                  ? serverToolTimeout * 1000
                  : 300000; // 默认5分钟
              logDebug(
                `[MCP Timeout] Calling tool "${tool.name}" from server "${serverName}" with ${timeout}ms timeout`,
              );
              ...
              logDebug(`[MCP Timeout] Tool "${tool.name}" completed in ${elapsed}ms`);
