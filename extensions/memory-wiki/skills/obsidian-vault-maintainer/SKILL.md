...
- Default to `openclaw wiki obsidian search <query>`, then `openclaw wiki obsidian open <vault-relative-path>`, and use `openclaw wiki obsidian daily` for daily-note navigation.
- Use `openclaw wiki obsidian command <id>` only when command-palette automation is explicitly required.
