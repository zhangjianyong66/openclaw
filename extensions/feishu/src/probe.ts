...
const FEISHU_PROBE_REQUEST_TIMEOUT_DEFAULT_MS = 30_000;
const FEISHU_PROBE_REQUEST_TIMEOUT_ENV_VAR = "OPENCLAW_FEISHU_PROBE_TIMEOUT_MS";

function resolveProbeRequestTimeoutMs(): number {
  const raw = process.env[FEISHU_PROBE_REQUEST_TIMEOUT_ENV_VAR];
  if (raw) {
  ...
  return FEISHU_PROBE_REQUEST_TIMEOUT_DEFAULT_MS;
}

export const FEISHU_PROBE_REQUEST_TIMEOUT_MS = resolveProbeRequestTimeoutMs();
